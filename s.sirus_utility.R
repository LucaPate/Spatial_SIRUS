...
rfgls.stab <- function(data.bin.y, num.trees.step = 1000, alpha = 0.05, mtry = NULL, max.depth = 2, 
  ...
  if (is.null(num.trees)){
    ...
    while (stab.metric < (1 - alpha)){
      forest <- rfgls(y ~., data = data.bin.y, num.trees = num.trees.step, mtry = mtry, max.depth = max.depth,
                         ...
                         coords = coords, incl.coords = incl.coords)
    ...
    forest <- rfgls(y ~., data = data.bin.y, num.trees = num.trees, mtry = mtry, max.depth = max.depth,
                       importance = 'none', oob.error = FALSE, write.forest = FALSE, replace = replace,
                       sample.fraction = sample.fraction, num.threads = num.threads, seed = seed,
                       coords = coords, incl.coords = incl.coords)
    paths <- forest$paths
...
s.sirus.param.check <- function(data, num.rule.max, q, num.trees.step, alpha, mtry){
  ...
  if (!num.rule.max.valid){
    ...
    if (num.rule.max > 100){
      warning('Warning num.rule.max: Spatial SIRUS is designed to output short list of rules (typically < 100 rules).')
    }
  }
  # check q
...
s.sirus.model.check <- function(s.sirus.m){
  ...
  s.sirus.valid <- FALSE
  type.valid <- is.list(s.sirus.m)
  if (type.valid){
    names.valid <- all(names(s.sirus.m) == c('rules', 'rules.out', 'proba', 'paths', 
                                           'rule.weights', 'rule.glm', 'type', 'num.trees', 'data.names', 'mean', 'bins'))
    if (names.valid){
      rules.valid <- all(sapply(s.sirus.m$rules, function(rule){
        is.list(rule) & 
          all(sapply(rule, function(split){
            if (split[2] == '='){length(split) >= 3}else{length(split) == 3}
          }))
      }))
      rules.out.valid <- all(sapply(s.sirus.m$rules.out, function(rule.out){
        is.list(rule.out) &
          all(names(rule.out) == c('outputs', 'supp.size')) &
          is.numeric(unlist(rule.out))
      }))
      proba.valid <- is.numeric(s.sirus.m$proba) & all(s.sirus.m$proba >= 0) & all(s.sirus.m$proba <= 1)
      mean.valid <- is.numeric(s.sirus.m$mean)
      weights.valid <- is.numeric(s.sirus.m$rule.weights)
      s.sirus.valid <- type.valid & names.valid & rules.valid & rules.out.valid &
        proba.valid & mean.valid
    }
  }
  return(s.sirus.valid)
